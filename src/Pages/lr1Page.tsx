...
import { AlertTriangle, CheckCircle2, ChevronDown, ChevronRight, Compass, Lightbulb, Pin } from 'lucide-react';
import type { Grammar } from '../Grammars/types';
import { normalizeGrammar } from '../Grammars/utils';
import { LR1Parser } from '../Parsers/lr1/lr1';
import type { LR1ParseStep } from '../Parsers/lr1/lr1';
import { LR1ItemSet } from '../Parsers/lr1/lr1Items';
import { GrammarInput } from '../Components/Common/grammarInput';
import { ResultPanel } from '../Components/Common/resultPanel';
import { LR1StatesView } from '../Components/LR1/lr1StatesView';
import { LR1TablesView } from '../Components/LR1/lr1Tables';
import { LR1Simulation } from '../Components/LR1/lr1Simulation';
...
type LR1Tab = 'states' | 'tables' | 'simulation';

export function LR1Page() {
  ...
  const [lr1Parser, setLr1Parser] = useState<LR1Parser | null>(null);
  const [itemSet, setItemSet] = useState<LR1ItemSet | null>(null);
  const [parseResult, setParseResult] = useState<{ accepted: boolean; steps: LR1ParseStep[]; error?: string } | null>(null);
  const [activeTab, setActiveTab] = useState<LR1Tab>('states');
  ...
  const buildTransitionMap = (parser: LR1Parser): Map<string, Map<string, number>> => {
  ...
  const buildPathToState = (parser: LR1Parser, targetState: number | null): number[] | undefined => {
  ...
  const handleAnalyze = () => {
    ...
    const parser = new LR1Parser(grammar);
    const items = new LR1ItemSet(grammar);
    setLr1Parser(parser);
    setItemSet(items);
    setParseResult(null);
  };
  ...
  const handleParse = () => {
    ...
    try {
      const result = lr1Parser.parse(inputString);
      console.log('Resultado LR(1):', result);
      setParseResult(result);
      setActiveTab('simulation');
      saveToHistory(grammarText, inputString, 'LR(1)', result.accepted);
    } catch (error) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <ResultPanel<LR1Tab>
        ...
        tabs={[
          { id: 'states', label: 'Estados LR(1)', disabled: !lr1Parser },
          { id: 'tables', label: 'Tablas LR(1)', disabled: !lr1Parser },
          { id: 'simulation', label: 'Simulación', disabled: !parseResult }
        ]}
      >
        {activeTab === 'states' && lr1Parser && itemSet && (
          <LR1StatesView
            states={lr1Parser.getStates()}
            itemSet={itemSet}
          />
        )}

        {activeTab === 'tables' && lr1Parser && parsedGrammar && (
          <div className="animate-fadeIn">
            ...
            <div ref={tableRef}>
          <LR1TablesView
            actionTable={lr1Parser.getActionTable()}
            gotoTable={lr1Parser.getGotoTable()}
            terminals={Array.from(parsedGrammar.terminals)}
            nonTerminals={Array.from(parsedGrammar.nonTerminals)}
            numStates={lr1Parser.getStates().length}
          />
          </div>
          </div>
        )}
        ...
        <div className="bg-pink-900/20 border border-pink-500/50 rounded-lg p-4">
          <h4 className="text-sm font-semibold text-pink-400 mb-2 flex items-center gap-2">
            <Lightbulb className="w-4 h-4" /> Sobre LR(1)
          </h4>
            <p className="text-sm text-gray-300">
                LR(1) es el parser más potente de la familia LR. Utiliza items con lookahead específico,
                lo que permite resolver conflictos que SLR(1) no puede manejar.
            </p>
            <p className="text-sm text-gray-300 mt-2">
            <span className="text-yellow-400 inline-flex items-center gap-1"><Pin className="w-4 h-4" />Características:</span><br/>
                • Items LR(1) con símbolo de lookahead<br/>
                • Puede manejar todas las gramáticas LR<br/>
                • Construcción más compleja<br/>
                • Mayor poder de reconocimiento
            </p>
            <p className="text-sm text-gray-300 mt-2">
            <span className="text-yellow-400 inline-flex items-center gap-1"><CheckCircle2 className="w-4 h-4" />Ventajas:</span> Máximo poder de reconocimiento, sin conflictos en gramáticas no ambiguas<br/>
            <span className="text-yellow-400 inline-flex items-center gap-1"><AlertTriangle className="w-4 h-4" />Desventajas:</span> Genera muchos estados (explosión exponencial)
            </p>
        </div>
        ...
        <div className="bg-slate-800/60 backdrop-blur-lg rounded-xl border border-slate-700 overflow-hidden shadow-xl">
          <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-200 flex items-center gap-2">
              <Compass className="w-4 h-4" /> Autómata LR(1)
            </h3>
            <button
          ...
          {showAutomaton && lr1Parser && (
            <div className="p-6 space-y-4">
              <LRAutomatonView
                states={lr1Parser.getStates()}
                transitions={buildTransitionMap(lr1Parser)}
                title="Autómata LR(1)"
                type="lr1"
                currentState={selectedState ?? undefined}
                highlightPath={buildPathToState(lr1Parser, selectedState)}
              />
              ...
              <div className="mt-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    type="range"
                    min={0}
                    max={Math.max(0, lr1Parser.getStates().length - 1)}
                  ...
                  />
                  <button
                    onClick={() => setSelectedState(Math.min(lr1Parser.getStates().length - 1, (selectedState ?? 0) + 1))}
                    disabled={selectedState === null || (selectedState ?? 0) >= lr1Parser.getStates().length - 1}
                    className="px-2 py-1 rounded bg-slate-700 hover:bg-slate-600 text-white text-xs disabled:opacity-40"
                  >
                    ▶
                  </button>
              ...
              {selectedState !== null && lr1Parser.getStates()[selectedState] && (
                <LRClosureView
                  state={lr1Parser.getStates()[selectedState]}
                  stateIndex={selectedState}
                  itemToString={lr1Parser.getItemSet().itemToString.bind(lr1Parser.getItemSet())}
                  isCurrent={true}
                />
              )}
            </div>
