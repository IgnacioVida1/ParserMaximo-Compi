...
const buildParseTree = (steps: RecursiveStep[], startSymbol: string): TreeNode => {
    let nodeId = 0;
    ...
    for (const step of steps) {
        if (step.production && step.success && step.production.includes('→')) {
            const parts = step.production.split('→');
        ...
        } else if (step.action.includes("Terminal") && step.action.includes('coincide')) {
            const match = step.action.match(/Terminal '([^']+)' coincide/);
            const terminal = match?.[1];
            if (terminal && nodeStack.length > 0 && nodeStack[nodeStack.length - 1].label === terminal) {
                nodeStack.pop();
            }
        }
    }
...
export function RecursiveParseTreeView({ steps, startSymbol, terminals }: Props) {
        ...
        <div className="bg-slate-900/60 rounded-xl p-5 border border-slate-700">
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-300 flex items-center gap-2">
                    <TreePine className="w-4 h-4" /> Arbol de parseo (Recursive)
                </h3>
