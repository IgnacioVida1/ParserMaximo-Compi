import type { LALR1Action } from '../../Parsers/lalr1/lalr1';

interface Props {
  actionTable: Map<number, Map<string, LALR1Action>>;
...
export function LALR1TablesView({ actionTable, gotoTable, terminals, nonTerminals, numStates }: Props) {
  ...
  if (numStates === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center bg-slate-900/60 border border-slate-700 rounded-lg px-4">
        ...
        <p className="text-blue-400 text-sm mt-2">Verifica que la gramática sea válida</p>
      </div>
    );
  }
