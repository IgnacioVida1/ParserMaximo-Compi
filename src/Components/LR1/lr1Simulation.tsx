...
export function LR1Simulation({ steps, accepted, error }: Props) {
  ...
  return (
    <div className="space-y-4 text-gray-400">
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
          ...
          <div className="flex flex-wrap gap-2">
            {step.stack.map((state, idx) => (
              <span key={idx} className="px-3 py-1 bg-purple-500/20 text-purple-300 rounded-lg font-mono text-sm">
                {state}
              </span>
            ))}
          </div>
      ...
      <details className="mt-4">
        <summary className="cursor-pointer text-sm text-purple-400 hover:text-purple-300 inline-flex items-center gap-2">
        ...
        <div className="mt-3 overflow-x-auto max-h-[300px] overflow-y-auto">
          <table className="min-w-full bg-slate-900 rounded-lg overflow-hidden text-sm">
            <thead className="sticky top-0 bg-slate-800">
              <tr className="bg-gradient-to-r from-purple-600/40 to-pink-600/40">
            ...
            <tbody>
              {steps.map((s, idx) => (
                <tr key={idx} className={`border-t border-slate-700 ${
                  idx === currentStep ? 'bg-purple-900/30' : ''
                }`}>
